...
def fetch_or_load(symbol, date):
    # Try basket sweep location first (Repo Root Data)
    sweep_path = REPO_ROOT / f"data/basket_sweep/{symbol}_{date}.csv"
    if sweep_path.exists():
        # print(f"[{symbol}] Loading from sweep cache...")
        return pd.read_csv(sweep_path)
    
    # Try genome location (Local Phase Data)
    # Check manifest destination structure: data/samples/local/{provider}/{symbol}/{date}
    # But wait, original code looked in OUT_DIR (which was data).
    # Let's check a flat file in data/ first, or the deeper structure.
    # For now, let's keep it simple: check flat in DATA_DIR or the deep structure?
    # The manifest downloads to: data/samples/local/polygon/{symbol}/{date}/trades.csv (likely)
    
    # Check deep structure
    deep_path = DATA_DIR / "samples" / "local" / "polygon" / symbol / date / "trades.csv"
    if deep_path.exists():
        return pd.read_csv(deep_path)
    
    # Check flat in DATA_DIR (legacy behavior support)
